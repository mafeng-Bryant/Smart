    ...
    if (sectionObject.type == SPAccessoryTableViewSectionTypeStatus) {
        if (rowObject.type == SPAccessoryTableViewRowTypeStatus) {
           SPAccessoryStatusCell *cell = [tableView dequeueReusableCellWithIdentifier:[SPAccessoryStatusCell reuseIdentifier] forIndexPath:indexPath];
            ...
            [cell configureData:self.accessory];
...
{
    if (!_headView) {
        _headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.tableView.frame.size.width, 40)];
        ...
        [statusLbl sizeToFit];
        statusLbl.frame = CGRectMake((self.tableView.frame.size.width - statusLbl.frame.size.width)/2.0, 16,statusLbl.frame.size.width, statusLbl.frame.size.height);
