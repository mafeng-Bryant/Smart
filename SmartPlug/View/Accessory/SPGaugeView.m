...
@interface SPGaugeView()

@end

@implementation SPGaugeView
{
    CGRect fullRect;
    CGRect innerRimRect;
...
- (void)initialize;
{
    ...
    _innerBackgroundStyle = SPGaugeViewInnerBackgroundStyleGradient;
    
    _needleWidth = 0.035;
    _needleHeight = 0.34;
    _needleScrewRadius = 0.04;
    _needleStyle = SPGaugeViewNeedleStyle3D;
    _needleScrewStyle = SPGaugeViewNeedleScrewStyleGradient;
    ...
    _scalesubdivisionsaligment = SPGaugeViewSubdivisionsAlignmentTop;
    ...
    _rangeLabels = nil;
//    
//    _scaleDivisionColor = RGB(68, 84, 105,1.0);
//    _scaleSubDivisionColor = RGB(217, 217, 217,1.0);
...
- (void)drawFace:(CGContextRef)context
{
    ...
    {
        case SPGaugeViewInnerBackgroundStyleGradient:
        {
            // Default Face
            CGColorSpaceRef baseSpace = CGColorSpaceCreateDeviceRGB();
            CGGradientRef gradient = CGGradientCreateWithColors(baseSpace, (CFArrayRef)@[iCGRGBA(96, 96, 96,1.0), iCGRGBA(68, 68, 68,1.0), iCGRGBA(32, 32, 32,1.0)], (const CGFloat[]){0.35, 0.96, 0.99});
        ...
        case SPGaugeViewInnerBackgroundStyleFlat:
        {
...
#define INTERNAL_RING_RADIUS    0.1
            ...
            CGContextSetFillColorWithColor(context, CGRGBA(255, 104, 97,1.0));
            ...
            CGContextSetFillColorWithColor(context, CGRGBA(242, 99, 92,1.0));
            CGContextFillPath(context);
        }
            break;
...
- (void)drawText:(CGContextRef)context
{
    ...
    [attrStr drawAtPoint:CGPointMake(0.5 - fontWidth.width / 2.0, 0.64)];
}

- (void)drawEneygy:(CGContextRef)context
{
    CGContextSetShadow(context, CGSizeMake(0.05, 0.05), 1.0);
    UIFont* font = [UIFont fontWithName:@"Helvetica" size:0.08];
    NSDictionary* stringAttrs = @{ NSFontAttributeName : font, NSForegroundColorAttributeName : [UIColor yellowColor]};
    NSAttributedString* attrStr = [[NSAttributedString alloc] initWithString:_showEnergy attributes:stringAttrs];
    CGSize fontWidth = [_showEnergy sizeWithAttributes:stringAttrs];
    [attrStr drawAtPoint:CGPointMake(0.5 - fontWidth.width / 2.0, 0.8)];
}
...
- (void)drawScale:(CGContextRef)context
{
    ...
    {
        CGFloat offset = 0.0;
        if (_scalesubdivisionsaligment == SPGaugeViewSubdivisionsAlignmentCenter) offset = (_scaleDivisionsLength - _scaleSubdivisionsLength) / 2.0;
        if (_scalesubdivisionsaligment == SPGaugeViewSubdivisionsAlignmentBottom) offset = _scaleDivisionsLength - _scaleSubdivisionsLength;
...
- (void)drawNeedle:(CGContextRef)context
{
    ...
    switch (_needleStyle)
    {
        case SPGaugeViewNeedleStyle3D:
        {
            ...
            [RGB(252, 18, 30,1.0) setFill];
            [rightNeedlePath fill];
        }
            break;
            
        case SPGaugeViewNeedleStyleFlatThin:
        {
...
#define SHADOW_OFFSET  0.008
            ...
            [RGB(255, 104, 97,1.0) setFill];
            [RGB(255, 104, 97,1.0) setStroke];
            [needlePath fill];
            needlePath.lineWidth = 0.004;
            [needlePath stroke];
        }
...
- (void)drawNeedleScrew:(CGContextRef)context
{
    ...
    {
        case SPGaugeViewNeedleScrewStyleGradient:
        {
            // Screw
            CGColorSpaceRef baseSpace = CGColorSpaceCreateDeviceRGB();
            CGGradientRef gradient = CGGradientCreateWithColors(baseSpace, (CFArrayRef)@[iCGRGBA(171, 171, 171,1.0), iCGRGBA(255, 255, 255,1.0), iCGRGBA(171, 171, 171,1.0)], (const CGFloat[]){0.05, 0.9, 1.00});
            (void)(CGColorSpaceRelease(baseSpace)), baseSpace = NULL;
            ...
            // Border
            CGContextSetLineWidth(context, 0.005);
            CGContextSetStrokeColorWithColor(context, CGRGBA(81, 84, 89, 1));
        ...
        case SPGaugeViewNeedleScrewStylePlain:
        {
            CGContextAddEllipseInRect(context, CGRectMake(center.x - _needleScrewRadius, center.y - _needleScrewRadius, _needleScrewRadius * 2.0, _needleScrewRadius * 2.0));
            CGContextSetFillColorWithColor(context, CGRGBA(68, 84, 105,1.0));
            CGContextFillPath(context);
        }
            break;
...
- (void)setInnerBackgroundStyle:(SPGaugeViewInnerBackgroundStyle)innerBackgroundStyle
{
...
- (void)setNeedleStyle:(SPGaugeViewNeedleStyle)needleStyle
{
    _needleStyle = needleStyle;
    [self setNeedsDisplay];
}

- (void)setNeedleScrewStyle:(SPGaugeViewNeedleScrewStyle)needleScrewStyle
{
...
- (void)setScalesubdivisionsaligment:(SPGaugeViewSubdivisionsAlignment)scalesubdivisionsaligment
{
...
-(void)setShowEnergy:(NSString *)showEnergy
{
    _showEnergy = showEnergy;
    [self invalidateBackground];
}
