...
@interface SPDatePicker()
{
    ...
    //限制model,设置时间的最大值和最小值
    SPDatePicker_DateModel *maxDateModel;
    SPDatePicker_DateModel *minDateModel;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.maxLimitDate) {
        maxDateModel = [[SPDatePicker_DateModel alloc]initWithDate:self.maxLimitDate];
    }else{
    ...
    if (self.minLimitDate) {
        minDateModel = [[SPDatePicker_DateModel alloc]initWithDate:self.minLimitDate];
    }else{
    ...
    if (!myPickerView) {
        myPickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(-10, 0, self.frame.size.width, self.frame.size.height)];
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    SPDatePicker_DateModel *model = [[SPDatePicker_DateModel alloc]initWithDate:dateShow];
    ...
    if (self.datePickerStyle == SPDateStyle_YearMonthDayHourMinute)
        return @[year,month,day,hour,minute];
    if (self.datePickerStyle == SPDateStyle_YearMonthDay)
        return @[year,month,day];
    if (self.datePickerStyle == SPDateStyle_MonthDayHourMinute)
        return @[month,day,hour,minute];
    if (self.datePickerStyle == SPDateStyle_HourMinute)
        return @[hour,minute];
    return nil;
}
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x-10, 98, 20, 20)];
    label.text = name;
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component
{
    switch (self.datePickerStyle) {
        case SPDateStyle_YearMonthDayHourMinute:{
